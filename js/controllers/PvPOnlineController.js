...
export default class OnlinePvPGameController {
  ...
  try {
    ...
    if (data.winner) {
      ...
      if (this.serverManager && typeof this.serverManager.closeUpdate === 'function') {
        this.serverManager.closeUpdate();
      } else if (this.serverManager && typeof this.serverManager.stopListening === 'function') {
  ...
  async handleBoardClick(row, col) {
    ...
    if (this.localSelectedFromIndex != null) {
      ...
      try {
        // Enviar origem e depois destino ao servidor
  ...
  handleGameOver(winner) {
    ...
    this.openScoreboardPanel(this.game.columns);
  ...
  async loadOnlineScores() {
      try {
          const size = this.game ? this.game.columns : 7;
          const ranking = await this.serverManager.ranking(this.serverManager.GROUP_ID,size);
          this.uiManager.renderOnlineScoreboard(ranking);
      } catch (err) {
          this.uiManager.setMessage("Error loading online ranking.");
          console.error(err);
      }
  }
