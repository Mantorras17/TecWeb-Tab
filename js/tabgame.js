...
export default class TabGame {
  ...
  /**
   * If no legal move exists for the current roll, auto-end the turn (respecting extra turns).
   * @returns {boolean} True if a skip happened.
   */
  autoSkipIfNoMoves() {
    if (this.stickValue != null && !this.hasAnyLegalMove()) {
      const keepTurn = this.playAgain();
      if (keepTurn) this.endTurn(true);
      else this.endTurn(false);
      return true;
    }
    return false;
  }
  ...
  /**
   * Handle capturing an opponent piece (remove it, play sfx, check end).
   * @param {Piece} capturedPiece
   * @returns {void}
   */
  handleCapture(capturedPiece) {
    try { window.sfx?.play('capture'); } catch {}
  ...
  moveSelectedTo(row, col) {
    ...
    try { window.sfx?.play('move'); } catch {}
    this.clearSelection();
